...
if config["reverse"]:
    rule reconstruct:
        ...
        binaries/basic_reconstruction --input {input.bam} --output {output.bam} --gtf {params.gff} --sample-map {input.sample_map} --threads {threads} --gene-identifier {config[gff_gene_identifier]} --reverse > {log} 2>&1
        touch {output.done}
        """
else:
    rule reconstruct:
        ...
        shell:"""
        echo Step 6/7 Reconstruct Molecules
        binaries/basic_reconstruction --input {input.bam} --output {output.bam} --gtf {params.gff} --sample-map {input.sample_map} --threads {threads} --gene-identifier {config[gff_gene_identifier]} > {log} 2>&1
        touch {output.done}
        """
