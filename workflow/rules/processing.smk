...
rule trim_fastq:
    ...
    shell: "cutadapt -j {threads} --json {log.summary} {config[params][cutadapt]} --too-short-output {output.r1_short} --too-short-paired-output {output.r2_short} -o {output.r1} -p {output.r2} {input.r1} {input.r2} > {log.stdout} 2>&1"
...
rule split_bam_by_strand:
    ...
    conda: "../envs/full.yaml"
    shell: "python3 workflow/scripts/split_bam_by_strand.py {input} > {log} 2>&1"
