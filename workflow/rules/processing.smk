rule make_barcode_files:
    ...
    shell: "python3 workflow/scripts/make_sample_files.py -s {input} --index-sequences {params.index_sequences} --sample_barcodes {output.barcodes} --cell_barcodes {output.cell_barcodes} --sample_map {output.sample_map} --readtype_map {output.readtype_map} --samplesheet-out {output.samplesheet_out}"
...
rule assign_genes_exon:
    ...
    output: pstrand = temp("results/{name}.trimmed.aligned.pstrand.GeneTagged.Exon.bam".format(name=config["name"])),
    ...
    conda: "../envs/full.yaml"
    shell:"""
    featureCounts -a {params.gtffile_positive} -t exon --primary  -T {threads} -R {output.pstrand} -p −−countReadPairs −−largestOverlap −−fracOverlap 0.1  {input.pstrand}
    featureCounts -a {params.gtffile_negative} -t exon --primary  -T {threads} -R {output.mstrand} -p −−countReadPairs −−largestOverlap −−fracOverlap 0.1  {input.mstrand}
    featureCounts -a {params.gtffile} -t exon --primary  -T {threads} -R {output.nostrand} -p −−countReadPairs −−largestOverlap −−fracOverlap 0.1  {input.nostrand}
...
rule assign_genes_intron:
    ...
    conda: "../envs/full.yaml"
    shell:"""
    featureCounts -a {params.gtffile_positive} -t intron --primary  -T {threads} -R {output.pstrand} -p −−countReadPairs −−largestOverlap −−fracOverlap 0.1  {input.pstrand}
    featureCounts -a {params.gtffile_negative} -t intron --primary  -T {threads} -R {output.mstrand} -p −−countReadPairs −−largestOverlap −−fracOverlap 0.1  {input.mstrand}
    featureCounts -a {params.gtffile} -t intron --primary  -T {threads} -R {output.nostrand} -p −−countReadPairs −−largestOverlap −−fracOverlap 0.1  {input.nostrand}
    mkdir -p results/.tmp_bgab/
    """
