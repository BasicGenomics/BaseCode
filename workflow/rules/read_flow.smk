...
rule fastq_processed:
    ...
    output: stats = "results/read_flow_files/{sample}/{sample}_fastq_processed_stats.json",
            done  = "results/done/{sample}.fastq_processed.done"
    threads: int(config["threads"])-1
    log: "results/read_flow_files/logs/{sample}/{sample}.fastq_processed.log"
    shell: """
    binaries/analyze_fastq --read1 {input.r1} --read2 {input.r2} --cbcpath {input.cbcpath} --pbcpath {input.pbcpath} --threads {config[threads]} --output {output.stats}  > {log} 2>&1
    touch {output.done}
    """
...
rule fastq_trimmed:
    input: r1 = "results/intermediate/{sample}.trimmed.read1.fastq.gz",
           ...
           cbcpath = "results/metadata/{name}_cell_barcodes.txt".format(name=config["name"]),
           pbcpath = "results/metadata/{name}_sample_barcodes.txt".format(name=config["name"])
    output: stats = "results/read_flow_files/{sample}/{sample}_fastq_trimmed_stats.json",
            done  = "results/done/{sample}.fastq_trimmed.done"
    threads: int(config["threads"])-1
...
rule fastq_too_short:
    input: r1 = "results/intermediate/{sample}.tooshort.read1.fastq.gz",
           ...
           cbcpath = "results/metadata/{name}_cell_barcodes.txt".format(name=config["name"]),
           pbcpath = "results/metadata/{name}_sample_barcodes.txt".format(name=config["name"])
    output: stats = "results/read_flow_files/{sample}/{sample}_fastq_tooshort_stats.json",
            done  = "results/done/{sample}.fastq_too_short.done"
    threads: int(config["threads"])-1
...
rule bam_geneassigned:
    ...
    output: mapping_group = "results/read_flow_files/{sample}/{sample}_mapping_group.csv",
            done          = "results/done/{sample}.bam_geneassigned.done"
...
rule bam_reconstructed:
    ...
    output: status_group = "results/read_flow_files/{sample}/{sample}_status_group.csv",
            done         = "results/done/{sample}.bam_reconstructed.done"
    conda: "../envs/full.yaml"
