...
rule overlap_and_mi:
    ...
    output: genes_out = "results/QC_files/{name}_overlap_and_mi_genes.csv".format(name=config["name"]), 
            cells_out = "results/QC_files/{name}_overlap_and_mi_cells.csv".format(name=config["name"])
    log: "results/logs/overlap_and_mi.log"
    params: gff = "{}.gff3".format(GFF)
    threads: config["threads"]
    shell: "python3 workflow/scripts/overlap_and_mi.py -i {input.bam} -g {params.gff} --genes-out {output.genes_out} --cells-out {output.cells_out} -t {threads} --gene-identifier {config[gff_gene_identifier]} > {log} 2>&1"
...
rule status_stats:
    ...
    params: gff = "{}.gff3".format(GFF)
    threads: config["threads"]
    shell: "python3 workflow/scripts/count_status_per_gene.py -i {input.bam} -g {params.gff} --counts {output.counts_per_gene} --sum {output.sum_over_genes} --fraction {output.fraction_per_gene} -t {threads} --gene-identifier {config[gff_gene_identifier]} > {log} 2>&1"
