rule count_lengths:
    ...
    shell: "python3 workflow/scripts/reconstruction_lengths.py -i {input.bam} -o {output} --gene-identifier {config[gff_column_name]} > {log} 2>&1"
...
rule overlap_and_mi:
    ...
    shell: "python3 workflow/scripts/overlap_and_mi.py -i {input.bam} -g {params.gtf} --genes-out {output.genes_out} --cells-out {output.cells_out} -t {threads} --gene-identifier {config[gff_column_name]} > {log} 2>&1"
...
rule status_stats:
    ...
    shell: "python3 workflow/scripts/count_status_per_gene.py -i {input.bam} -g {params.gtf} --counts {output.counts_per_gene} --sum {output.sum_over_genes} --fraction {output.fraction_per_gene} -t {threads} --gene-identifier {config[gff_column_name]} > {log} 2>&1"
